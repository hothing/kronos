...
void IGD480::copyBitmap()
{
    ...
    //uint32_t* pal = (uint32_t*)(uint8_t*)&mem[IGD480base + 0x10];
    uint8_t palette[16][3];
    for (int i = 0; i < 16; i++)
    {
        assert(ptrans[ptrans[i]] == i);
        int c = mem[IGD480base + 0x10 + i]; // FIXME: this expression is wrong:
                                            // get a uint8_t and convert to
                                            // uint32_t, while we expect to get uint32_t value
        int r = ptrans[c % 16] / 4;
