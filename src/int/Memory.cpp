    ...
    bOutOfRange(false)
{
    ...
    //pReservered = (uint8_t*)::VirtualAlloc(null, nSizeWithIGD * 4, MEM_RESERVE, PAGE_READWRITE);

    //data = (int*)::VirtualAlloc(pReservered, nMemorySize*4, MEM_COMMIT,  PAGE_READWRITE);
    assert(pReservered == (uint8_t*)data);
//  trace("Memory: %08x\n", data);

    void* pIGDregisters = nullptr; //::VirtualAlloc(pReservered + IGD480base*4, 4*K, MEM_COMMIT,  PAGE_READWRITE);
    assert(pIGDregisters == pReservered  + IGD480base*4);
    (void)pIGDregisters;

    void* pIGDbitmap = nullptr; //::VirtualAlloc(pReservered + IGD480bitmap*4, IGD480size * 4, MEM_COMMIT,  PAGE_READWRITE);
