...
igd(&mem, mouse, con)
{
  ...
  DWORD id = 0;
  hTimerThread = CreateThread(nullptr,  0, ThreadProc, this, 0, &id);
...
void VM::bitBlt(uint32_t * dst, uint32_t dofs, uint32_t * src, uint32_t sofs, uint32_t bits)
{
  ...
  // for byte-aligned bit address the standard copy function can be used
  ...
  // case: bit addresses are in-word aligned
  if (dofs == sofs && dofs != 0)
    {
...
void VM::BitBlt(uint32_t dst, uint32_t dofs, uint32_t src, uint32_t sofs, uint32_t bits)
{
  // FIXME: a code should be moved out  
  ...
  // FIXME:END
  dst = dst + (dofs >> 5); // fix-up the destination word address
  src = src + (sofs >> 5); // fix-up the source word address
...
void VM::BitMove(uint32_t dst, // destination word address 
		 uint32_t _dofs, // destination bit offset
		 uint32_t src, // source word address
		 uint32_t _sofs, // source bit offset
		 uint32_t bits // length of region
)
{
  /*     DWORD  test = mem[dst]  */
  /*                 | mem[src] */
  ...
  // FIXME:END
  dst = dst + (_dofs >> 5); // fix-up the destination word address
  src = src + (_sofs >> 5); // fix-up the source word address
  // fix-up the bit offset value in range [0 .. 31]
  uint32_t dofs = _dofs & 0x1F; // destanation bit offset in word
  uint32_t sofs = _sofs & 0x1F; // source bit offset in word
  
  // FIXME: a calculation should not use the int64
  int64_t qdst = (int64_t(dst) << 5) + _dofs; // destination bit address
  int64_t qsrc = (int64_t(src) << 5) + _sofs; // source bit address
  ...
  if ((sofs & 0x7) == 0 && (dofs & 0x7) == 0 && (bits & 0x7) == 0)
    { // byte-aligned addresses are given
      // standard function can be used
      memmove((uint8_t*)&mem[dst] + (dofs >> 3), (uint8_t*)&mem[src] + (sofs >> 3), bits >> 3);
      return;
    }

  dofs += bits - 1; // point to the end of destination region 
  sofs += bits - 1; // point to the end of source region
  uint32_t* pdst = (uint32_t*)(uint8_t*)&mem[dst];
  uint32_t* psrc = (uint32_t*)(uint8_t*)&mem[src];
  uint32_t dmsk = 0;
  uint32_t smsk = 0;
...
void VM::dch(int mode, Bitmap* bmp, int x, int y, Font* font, int ch)
{
  ...
  //unused(test);
  DWORD* F = (DWORD*)(uint8_t*)&mem[font->base + ch * h];
  DWORD* L = (DWORD*)(uint8_t*)&mem[bmp->base + y * bmp->wpl + (x >> 5)];
